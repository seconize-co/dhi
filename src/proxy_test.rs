...
mod proxy_ssrf_tests {
    ...
    fn test_block_ipv6_unique_local() {
        ...
        assert!(
            is_private_ip(ip),
            "Should block IPv6 unique local (fc00::/7)"
        );

        let ip: IpAddr = "fd00::1".parse().unwrap();
        assert!(
            is_private_ip(ip),
            "Should block IPv6 unique local (fd00::/8)"
        );
    }
    ...
    fn test_block_aws_metadata() {
        assert!(
            is_suspicious_hostname("169.254.169.254"),
            "Should block AWS metadata IP"
        );
    }
    ...
    fn test_block_gcp_metadata() {
        assert!(
            is_suspicious_hostname("metadata.google.internal"),
    ...
    fn test_block_azure_metadata() {
        assert!(
            is_suspicious_hostname("metadata.azure.com"),
            "Should block Azure metadata"
        );
    }
    ...
    fn test_block_internal_domains() {
        assert!(
            is_suspicious_hostname("secret.internal"),
            "Should block .internal"
        );
        assert!(
            is_suspicious_hostname("db.cluster.local"),
            "Should block .cluster.local"
        );
    ...
    fn test_block_localhost_variants() {
        assert!(
            is_suspicious_hostname("localhost"),
            "Should block localhost"
        );
        assert!(
            is_suspicious_hostname("LOCALHOST"),
            "Should block LOCALHOST (case-insensitive)"
        );
        assert!(
            is_suspicious_hostname("localhost.localdomain"),
            "Should block localhost.localdomain"
        );
        assert!(
            is_suspicious_hostname("evil.localhost"),
            "Should block *.localhost"
        );
    }
    ...
    fn test_block_docker_internal() {
        assert!(
            is_suspicious_hostname("host.docker.internal"),
            "Should block Docker host"
        );
        assert!(
            is_suspicious_hostname("gateway.docker.internal"),
            "Should block Docker gateway"
        );
    }
    ...
    fn test_allow_legitimate_domains() {
        assert!(
            !is_suspicious_hostname("api.openai.com"),
            "Should allow OpenAI"
        );
        assert!(
            !is_suspicious_hostname("api.anthropic.com"),
            "Should allow Anthropic"
        );
        assert!(
            !is_suspicious_hostname("example.com"),
            "Should allow example.com"
        );
        assert!(
            !is_suspicious_hostname("internal-api.company.com"),
            "Should allow 'internal' in path"
        );
    }
